...
public class StringUtil {
    ...
    /** 默认中文编码字符集 */
    public static final String DEFAULT_CHINESE_CHARSET = "UTF-8";
     ...
     * @return
     */
    public static String substringFixLength(String str, int size, String charset) {

        // 字符串没有超长，不需要截取处理
        if (str == null || getByteLength(str, charset) <= size) {
            return str;
        }

        // 判断长度位是否包含中文
        if (isContainChinese(str, size, charset)) {

            // 包含中文就少截取1位
            return substrings(str, 0, size - 1, charset);
        }

        // 长度位不是中文直接截取
        return substrings(str, 0, size, charset);
    }
     ...
     * @return
     */
    public static boolean isContainChinese(String message, int length, String charset) {
        char[] chars = StringUtil.substrings(message, 0, length, charset).toCharArray();
        char[] charsPlus = StringUtil.substrings(message, 0, length + 1, charset).toCharArray();
        return ArrayUtils.isSameLength(chars, charsPlus);
    }
    ...
    /**
     * 在字符串的左边填充长度信息，根据按字符编码转换成的byte长度的长度定义，采用左补零右对齐的方式
     *
     * @param input
     * @param lengthLength
     * @return
    ...
    public static String fillLength(String input, String charset, int lengthLength) {
        ...
        try {
            ...
            length = Integer.toString(input.getBytes().length);
        }
        String filledLength = StringUtil.alignRight(length, lengthLength, "0");
        return filledLength + input;
    }
    ...
    /**
     * 获取指定字符串按charset编码转换成的byte长度 由于String.getByte方法依赖于操作系统编码，处理中文字符串时建议用此方法
     *
     * @param data
     * @param charset
     * @return
     */
    public static byte[] getByte(String data, String charset) {
        if (data == null) {
            return new byte[0];
        }
        try {
     ...
     * @return
     */
    public static int getByteLength(String data, String charset) {
        return getByte(data, charset).length;
    }
...
//            throw new SupergwException(SystemErrorCode.SYSTEM_ERROR);
//        }
//    }
     ...
     *
     * <pre>
     * "userId=666"           = {userId=666}
     * "userId=666|name=loki" = {name=loki, userId=666}
     * "userId=666,name=loki" = {name=loki, userId=666}
     * </pre>
     *
     * @param data
     ...
     * @return
     */
    public static Map<String, String> parseStringToMap(String data, String token) {
        if (isBlank(data) || isEmpty(token)) {
            return null;
        }
        if (!contains(data, token)) {
            throw new NoSuchElementException("StringUtil.parseStringToMap data has no contains token!");
        }
        ...
        while (tokenizer.hasMoreTokens()) {
            ...
            if (element.indexOf(PROPERTY_DELIMER) != -1) {
                map.put(element.substring(0, element.indexOf(PROPERTY_DELIMER)),
     ...
     * @return
     */
    public static Map<String, String> parseStringToLinkedMap(String data, String token) {
        if (isBlank(data) || isEmpty(token)) {
            return null;
        }
        if (!contains(data, token)) {
